...
class COCOeval:
    ...
    def evaluate_fd(self):
        ...
        evaluateImg_fd = self.evaluateImg_fd
        maxDet = p.maxDets[-1]
        self.evalImgs = [evaluateImg_fd(imgId, catId, areaRng, maxDet)
                 for catId in catIds
                 for areaRng in p.areaRng
                 for imgId in p.imgIds
             ]
    ...
    def evaluateImg_fd(self, imgId, catId, aRng, maxDet):
        '''
        perform evaluation for single category and image
        :return: dict (single image results)
        '''
        ...
        dtIg = np.zeros((T,D))
        if not len(ious)==0:
            for tind, t in enumerate(p.iouThrs_fd):
                for dind, d in enumerate(dt):
                    # information about best match so far (m=-1 -> unmatched)
                    iou = min([t,1-1e-10])
                    m   = -1
